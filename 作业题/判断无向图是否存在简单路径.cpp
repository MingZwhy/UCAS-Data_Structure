...
bool Find_Path(Ptr_ALGraph G, ElemType vp1, ElemType vp2, bool * visited, int k)
{
    if(vp1==vp2 && k==0){
        return true;
    }
        ...
        while(visit!=NULL){
            if(visited[visit->vindex-1]==false && Find_Path(G,visit->vindex,vp2,visited,k-1)){
                return true;
            }
            visit=visit->next;
        }
...
int main(){
    ...
    if(Find_Path(G,vp1,vp2,visited,path_len)){
        cout << "yes";
    }
    else{
        cout << "no";
