...
typedef int VertexType;  //顶点类型，可为int或char
...
void MiniSpanTree_PRIM(Ptr_To_MGraph G, VertexType u)
{
    ...
    for(int i=0;i<G->vexnum;i++){
        if(i!=LocateVex(G,u)){
            if(i<G->vexnum-1 && !(i==G->vexnum-2 && LocateVex(G,u)==G->vexnum-1)){
                cout << dist[i].lowcost << ",";
            }
            else{
                cout << dist[i].lowcost;
...
int main(){
    ...
    Ptr_ArcType arcs = new ArcType[MAX_ARC_NUM]; 
    arcnum=0;
    c=cin.get();
    while(c!=EOF){
        cin >> arcs[arcnum].vex1;
        c=cin.get();
        cin >> arcs[arcnum].vex2;
        c=cin.get();
        cin >> arcs[arcnum].Arcval;
        arcnum++;
        c=cin.get();
    }

    G=CreateGraph(G,vexnum,vexs,arcnum,arcs,UDG,true,false);
    MiniSpanTree_PRIM(G,target);
    return 0;
}