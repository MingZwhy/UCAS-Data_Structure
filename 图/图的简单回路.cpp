...
bool IF_HuiLu(Ptr_ALGraph G);   //判断有向图或无向图中是否存在回路
...
{
    using namespace std;
    cout << "Try to find path from" << vp1 << "to" << vp2 << "in Graph G" << endl;
...
bool IF_HuiLu(Ptr_ALGraph G)
{
    for(int i=0;i<G->vexnum;i++){
        ElemType vp=G->head[i].vertex;
        if(Find_Path_h(G,vp,vp)){
            return true;
        }
    }
    return false;
